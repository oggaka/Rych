...
if (!process.env.TOKEN || !process.env.CLIENT_ID || !process.env.GUILD_ID) {
  console.log("❌ Falta TOKEN/CLIENT_ID/GUILD_ID nas variáveis de ambiente.");
  process.exit(1);
}
...
async function createTicket({ guild, member, parentCategoryId, tipo }) {
  ...
  if (existing) {
    return { error: `⚠️ Você já tem um ticket aberto: <#${existing.id}>` };
  }
  ...
  const ch = await guild.channels.create({
    ...
    topic: `ticket_user:${member.user.id} | ticket_num:${ticketNum} | opened_at:${openedAt}${
      tipo ? ` | tipo:${safeSlug(tipo)}` : ""
    }`,
  });
...
const cmdAddCargo = new SlashCommandBuilder()
  .setName("addcargo")
  .setDescription(
    "Adiciona um cargo permitido para ver este ticket (somente staff autorizado)"
  )
  .setDMPermission(false)
  .addRoleOption((o) =>
    o
...
client.on(Events.InteractionCreate, async (interaction) => {
  try {
    ...
    if (interaction.isChatInputCommand() && interaction.commandName === "embed") {
      ...
      return interaction.reply({
        content: `✅ Embed enviado em <#${canal.id}>`,
        flags: MessageFlags.Ephemeral,
      });
    }
    ...
    if (interaction.isChatInputCommand() && interaction.commandName === "ticketpainel") {
      ...
      if (!canalPainel?.isTextBased()) {
        return interaction.reply({
          content: "❌ canal_painel precisa ser um canal de texto.",
          flags: MessageFlags.Ephemeral,
        });
      }
      ...
      if (categoriaTickets && categoriaTickets.type !== ChannelType.GuildCategory) {
        return interaction.reply({
          content: "❌ categoria_tickets precisa ser uma CATEGORIA.",
          flags: MessageFlags.Ephemeral,
        });
      }
      ...
      return interaction.reply({
        content:
          ...
          `📌 Tickets vão para: ${
            categoriaTickets ? `categoria <#${categoriaTickets.id}>` : "SEM categoria (raiz)"
          }.`,
        flags: MessageFlags.Ephemeral,
      });
    }
    ...
    if (interaction.isChatInputCommand() && interaction.commandName === "addcargo") {
      ...
      if (!canUse) {
        return interaction.reply({
          content: "❌ Você não pode adicionar cargos neste ticket.",
          flags: MessageFlags.Ephemeral,
        });
      }
      ...
      if (!ALLOWED_ADD_ROLE_IDS.includes(role.id)) {
        return interaction.reply({
          content: "❌ Você só pode adicionar os 2 cargos permitidos.",
          flags: MessageFlags.Ephemeral,
        });
      }
      ...
      if (existing?.allow?.has(PermissionsBitField.Flags.ViewChannel)) {
        return interaction.reply({
          content: `⚠️ O cargo <@&${role.id}> já tem acesso a este ticket.`,
          flags: MessageFlags.Ephemeral,
        });
      }
      ...
      await interaction.reply({
        content: `✅ Cargo <@&${role.id}> adicionado neste ticket.`,
        flags: MessageFlags.Ephemeral,
      });
    ...
    if (interaction.isButton() && interaction.customId === "ticket_open") {
      ...
      return interaction.reply({
        content: `✅ Ticket criado: <#${res.channelId}>`,
        flags: MessageFlags.Ephemeral,
      });
    }
    ...
    if (interaction.isStringSelectMenu() && interaction.customId.startsWith("ticket_menu:")) {
      ...
      if (!cfg?.usarMenu) {
        return interaction.reply({
          content: "❌ Este painel não tem menu configurado.",
          flags: MessageFlags.Ephemeral,
        });
      }
    ...
    if (interaction.isButton() && interaction.customId === "ticket_close") {
      ...
      if (!channel?.topic?.includes("ticket_user:")) {
        return interaction.reply({
          content: "❌ Isso não parece um ticket.",
          flags: MessageFlags.Ephemeral,
        });
      }
      ...
      if (channel.name.startsWith("fechando-")) {
        return interaction.reply({
          content: "⚠️ Já está fechando...",
          flags: MessageFlags.Ephemeral,
        });
      }

      try {
        await channel.setName(`fechando-${channel.name}`.slice(0, 90));
      } catch {}

      await interaction.reply({
        content: "⏳ Fechando e gerando log .txt...",
        flags: MessageFlags.Ephemeral,
      });
